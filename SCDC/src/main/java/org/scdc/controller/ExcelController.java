...
public class ExcelController {
    ...
    public void downloadExcelReport(HttpServletResponse response) throws IOException {
    	...
    	headerRow.createCell(7).setCellValue("품목설명");
    	headerRow.createCell(8).setCellValue("재고수량");
    	headerRow.createCell(9).setCellValue("단가");
    	headerRow.createCell(10).setCellValue("재고금액");
    	...
    	for (ReportVO partList : list) {
    		...
    		row.createCell(7).setCellValue(partList.getRemark());
    		row.createCell(8).setCellValue(partList.getStockQuantity());
    		row.createCell(9).setCellValue(partList.getUnitPrice());
    		row.createCell(10).setCellValue(partList.getTotalPrice());
    	}
